...
from model1 import Encoder,Seq2Seq
...
with_att = True #決定是否使用attention mechanism ,選擇所用的model
...
class Train():
    ...
    def train(self):
        for fold, (train_idx,val_idx) in enumerate(self.splits.split(np.arange(len(self.dataset)))):
            ...
            if with_att:
                path = '/home/yunghuan/Desktop/PuncTBRNN/model/model_multi_head_attn.pth.tar'
            else:
                path = '/home/yunghuan/Desktop/PuncTBRNN/model/model_GRU.pth.tar'
...
def main():
    ...
    '''
    if with_att:
        encoder = Encoder(input_size,embedding_dim,hidden_dim,4,1,0.2,pretrained)
        #decoder = Decoder(hidden_dim,4,4,0.0)
        #model = Seq2Seq(encoder,decoder,hidden_dim)
        model = Seq2Seq(encoder,hidden_dim,1,1)
       
        save_path = '/home/yunghuan/Desktop/PuncTBRNN/model/model_multi_head_attn.pth.tar10'
    else:
        save_path = '/home/yunghuan/Desktop/PuncTBRNN/model/model_noattn.pth.tar'
        model = bi_GRU(input_size,embedding_dim,hidden_dim,2,4,pretrained)
    use_cuda = True
    if use_cuda:
        model = model.cuda()
    print(model)
    ...
    optimizer = torch.optim.Adam(model.parameters(),lr = 9e-4,weight_decay=0)#決定optimizer （更新weight 的方式）
    ...
    '''
    TBRNN = Train(dataset,model,criterion,optimizer,use_cuda,2,30,scheduler,punc_path,save_path,collate_fn,False,batch_size_times=8)
    TBRNN.train()
